...
The resulting ledger is not a platform export.

## Method

1. Scrolled the authenticated Posts surface to exhaustion and deduplicated
...
3. Ran monthly date-bounded authenticated searches as an independent discovery
   channel. X search recovered only 47 of 92 authored statuses and no additional
   IDs, so it was not treated as a completeness check.
...
5. Preserved status ID, date, account relationship, public text, mentions,
   hashtags, outbound links, interface-level visual tokens, and currently
   visible public metrics. Media assets and their public locators were not
...
The fresh Posts tab pass yielded 106 items. The fresh Replies tab pass supplied one additional reply,
[`722837286476390401`](https://x.com/CallNYCapp/status/722837286476390401),
that was absent from the Posts-tab inventory. Their 107-URL union matched the
preserved ledger exactly. Three count slots remain `not-recovered`; no status
IDs or content are inferred for them.
...
categories. Institutional accounts such as `@NYCCouncil`, `@NYCHousing`, and
`@NYCHA` are not counted as Council members; each recognition record resolves
to exactly one member account after those exclusions.

A separate direct-action standard supports the stronger reciprocal traction
claim. The full corpus exposed Ydanis Rodriguez's quote-amplification; combined
with the broader authenticated account search, this raises the recoverable floor
to eight sitting members: Peter Koo, Steven Matteo, Ruben Wills, Ydanis
Rodriguez, Rosie Mendez, Helen Rosenthal, Mathieu Eugene, and Margaret Chin.
That is individual-account engagement, not official NYC Council endorsement,
adoption, or use by eight offices.
...
The ledger records interface-level `visualTokens` for 87 statuses. Eighty-two
records carry the literal token `Image`; these are media-recovery leads. Five
additional records carry only emoji-like tokens. Those tokens are retained but
not interpreted as proof of attached media. None are media URLs, asset files,
image descriptions, or publication clearances. This pass did not archive the
visual assets. Future visual research should return to the 82 status pages or
another public archive, recover stable media locators where available, describe
what the images show, and review rights and represented people before using
them in the portfolio or a photo-editing brief.

## Claims promoted

- `CLM-CALLNYC-SOCIAL-ENGAGEMENT-ARCHITECTURE` is selected for the CallNYC
  case study. It makes Jamie's operating layer, the scale, and the intended
  institutional audience legible without converting tags into responses or
  issue rows into people.
- `CLM-CALLNYC-COUNCIL-ACCOUNT-ENGAGEMENT` now carries the eight-member
  recovered floor after direct authenticated verification of Ydanis Rodriguez's
  quote-post.
...
- Recover and review the visual media referenced by the 82 `Image`-indicator
  records; retain the five emoji-only token sets as uninterpreted signals.
