---
id: index.knowledge-wiki.pilot
title: Knowledge Wiki pilot map
kind: index
status: maintained
visibility: public-safe
sensitivity: low
last_reviewed: 2026-07-18
review_by: 2026-10-18
canonical_path: docs/knowledge-bank/indexes/pilot.md
summary: Task-oriented map of the bounded CallNYC and Technical Operations pilot.
relations:
  - type: related_to
    target: project.callnyc
    href: ../projects/callnyc.md
  - type: related_to
    target: event.nycc.constituent-services-hackathon.2016
    href: ../events/nycc-constituent-services-hackathon-2016.md
  - type: related_to
    target: claim.callnyc.independent-follow-on
    href: ../claims/callnyc-independent-follow-on.md
  - type: related_to
    target: capability.technical-operations
    href: ../capabilities/technical-operations.md
  - type: related_to
    target: method.source-backed-team-memory
    href: ../methods/source-backed-team-memory.md
  - type: related_to
    target: method.nyc-jobs-opportunity-loop
    href: ../methods/nyc-jobs-opportunity-loop.md
  - type: related_to
    target: opportunity.nyc-oti.technical-operations-manager.782369
    href: ../opportunities/oti-technical-operations.md
---

# Knowledge Wiki pilot map

This bounded cross-section tests the document, semantic, evidence, and
publication graphs without migrating the complete historical corpus.

## Understand the project

- [CallNYC](../projects/callnyc.md)
- [Council constituent-services hackathon](../events/nycc-constituent-services-hackathon-2016.md)
- [Independent follow-on claim](../claims/callnyc-independent-follow-on.md)
- [Chronology correction](../corrections/callnyc-years-2026.md)

## Inspect the source basis

- [Civic Hall announcement](../sources/civichall-hackathon-announcement-2016.md)
- [New York City Council event-day post](../sources/nycc-councilstat-hackathon-post-2016.md)
- [Politico New York coverage](../sources/politico-callnyc-2016-03-14.md)

## Preserve the boundary

- [Digital District photograph metadata](../assets/digital-district-photo.md)
- [CallNYC public projection](../projections/work-callnyc.md)

## Use the proof responsibly

- [Technical Operations capability](../capabilities/technical-operations.md)
- [Source-Backed Team Memory method](../methods/source-backed-team-memory.md)
- [NYC Jobs opportunity action loop](../methods/nyc-jobs-opportunity-loop.md)
- [Technical Operations opportunity lens](../opportunities/oti-technical-operations.md)
- [Technical Operations portfolio projection](../projections/work-technical-operations.md)
- [Retrieval tasks](../evaluations/retrieval-tasks.md)

## Operate the system

- [Record contract](../schema.md)
- [VS Code workflow](../workflow.md)
- [Generated indexes](../_generated/index-by-kind.md)
- [Health report](../../../reports/wiki-health.md)
