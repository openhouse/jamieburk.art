---
id: index.knowledge-wiki.decisions-deliverables-operational-outcomes
title: Decisions, Deliverables, and Operational Outcomes
kind: index
status: maintained
visibility: public-safe
sensitivity: moderate
last_reviewed: 2026-07-19
review_by: 2026-10-19
canonical_path: docs/knowledge-bank/indexes/decisions-deliverables-and-operational-outcomes.md
summary: Hiring-oriented map separating difficult conditions, Jamie's supported responsibility, usable deliverables, wider results, and evidence boundaries.
projection:
  status: hold
  surfaces: []
relations:
  - type: informed_by
    target: source.knowledge-wiki.branch-d.2026-07-19
    href: ../sources/knowledge-wiki-branch-d-2026-07-19.md
  - type: related_to
    target: index.knowledge-wiki.decision-records
    href: decision-records.md
  - type: related_to
    target: capability.technical-operations
    href: ../capabilities/technical-operations.md
  - type: related_to
    target: project.callnyc
    href: ../projects/callnyc.md
  - type: related_to
    target: project.nyc-artist-coalition
    href: ../projects/nyc-artist-coalition-2017.md
  - type: related_to
    target: project.kc-town-hall
    href: ../projects/kc-town-hall-orientation.md
  - type: related_to
    target: project.kc-spaces-fund
    href: ../projects/kc-spaces-fund-orientation.md
---

# Decisions, Deliverables, and Operational Outcomes

For each example, keep five layers separate: the difficult condition, Jamie's
supported responsibility, what became usable, the wider result, and what the
evidence does not establish.

| Work | Jamie's supported responsibility | What became usable | Wider result | Boundary |
| --- | --- | --- | --- | --- |
| CallNYC | Independently built a follow-on interpretation of released CouncilStat data | Resident-facing issue pathways and contextual next steps | Public reporting and bounded Council-account amplification | No Council adoption, usage total, constituent outcome, or official-product status |
| NYC Artist Coalition | Founding and early organizing role plus documented identity, implementation, and participation systems | Meetings, campaign routes, testimony support, practical resources, and public interfaces | Coalition work appears within broader policy and agency histories | Outcomes were collective and institutional; complete task allocation is unresolved |
| KC Town Hall | Planning, public-benefit documentation, daily Phase One coordination, and survey practice at the currently supported levels | Stabilization scope, survey workflow, presentation, and funding packet | Board recommendation and Council appropriation followed; later records show no disbursement and return of the unused amount | Government action is not Jamie's action, and appropriation is not receipt or completed redevelopment |
| KC Spaces Fund | Behind-the-scenes website, theme, widget, deployment, and action-route implementation | A coherent campaign site with donation, application, signup, contact, and fundraising display | The public fundraiser records its result | The result does not prove site causality or make Jamie an organizer or grant decision-maker |
| Source-Backed Team Memory | Designed a bounded source, privacy, review, prototype, and continue-revise-stop process | Inspectable implementation choices before production commitment | A reusable engagement design and current Wiki practice | Proposal and method do not establish client adoption or a production platform |

## Decision vocabulary

Use **decided** only when Jamie held the decision right; **recommended** when he
framed a course; **implemented** when direct delivery is supported;
**co-produced** or **helped establish** for collective work; **government
acted** for formal public authority; **the record shows** for an observable
result without causal proof; and **Jamie recalls** for bounded first-person
evidence.

Government action is not Jamie's action. A delivered artifact is not an adopted
recommendation. No row may collapse responsibility, deliverable, outcome,
adoption, and impact into one sentence.
