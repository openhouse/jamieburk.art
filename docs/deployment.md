...
dokku docker-options:add jamieburk-art build '--build-arg NEXT_PUBLIC_LINKEDIN_URL=<approved-url-or-empty>'
dokku docker-options:add jamieburk-art build '--build-arg NEXT_PUBLIC_GITHUB_URL=<approved-url-or-empty>'
```
...
- `/work/source-backed-team-memory` redirects to `/lab/source-backed-team-memory`.
- The resume PDF response includes `X-Robots-Tag: noindex, nofollow`.

Expected production behavior after final approval:
...
- `/api/health` exposes only `ok`, `service`, `siteUrl`, and
  `robotsIndexable`.
