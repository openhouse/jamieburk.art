...
const expectedRedirects = [
  ...
  ["/work/sunday-dinner-196-artists-residency", "/work/196-sunday-dinner"],
...
const baseInput = process.argv[2] ?? process.env.CHECK_ROUTES_BASE_URL ?? "http://localhost:3000";
...
for (const route of expectedRoutes) {
  ...
  if (route === "/sitemap.xml") {
    ...
    if (body.includes(toUrl("/lab/source-backed-team-memory"))) {
      failures.push("/sitemap.xml includes lab route without explicit approval");
    }
  }
