...
export function validateKnowledgeLifecycle(input = knowledgeLifecycle) {
  ...
  for (const observation of input.observations) {
    ...
    checkRefs(errors, `Observation ${observation.id} relationship`, relationshipIds, candidates, "candidate claim");
    if (new Set(relationshipIds).size !== relationshipIds.length) errors.push(`Observation ${observation.id} repeats a candidate relationship`);
    if (JSON.stringify([...observation.candidateClaimIds].sort()) !== JSON.stringify([...relationshipIds].sort())) {
      errors.push(`Observation ${observation.id} must define one candidate-specific relationship for every linked candidate`);
    }
