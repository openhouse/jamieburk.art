...
const isCredentialQueryKey = (value) =>
  ...
  /^(?:(?:access|api|auth|client|private|refresh|secret)[a-z0-9]*tokens?|(?:access|api|client|encryption|private|secret|signing)[a-z0-9]*keys?|credentials?|(?:meeting|room)[a-z0-9]*(?:codes?|ids?|pins?)|join[a-z0-9]*codes?|oauth[a-z0-9]*(?:codes?|tokens?)|session[a-z0-9]*(?:keys?|secrets?|tokens?)|verification[a-z0-9]*(?:codes?|keys?|tokens?))$/.test(
    value
  );
...
const publicMetadataPatterns = [
  ...
  [
    ...
    /(?:\b(?:attendees?|participants?)\s*(?::|\b(?:include|included|listed|named)\b)|\b(?:attendees?|participants?)\s+(?:were|are)\s+\p{Lu}[\p{L}'\u2019-]*|\b(?:attendee|participant)\s+lists?\b|\b(?:roster|list)\s+of\s+(?:attendees|participants)\b|\bguest\s+(?:lists?|roster)\b|\bnames?\s+of\s+guests?\b|\binvitees?\s*:|\brsvp\s+names?\s*:|\bwho\s+(?:came|attended)\s*:|\b(?:sign[ -]?in|sign[ -]?up)\s+sheets?(?:\s+names?)?\s*:|\broll\s+call\s*:|\bpeople\s+present\s*:|\bregistration\s+lists?\s*:|\bparticipant\s+director(?:y|ies)\s*:|\b(?:volunteer\s+roster|member\s+list|crew\s+manifest|stakeholder\s+directory|attendee\s+roll|guest\s+register)\s*:|\battendance\s+count\b|\bparticipant\s+roster\b|\bin confidence\b|\bconfidential\b|\bmedical history\b|\beviction history\b|\bprivate coalition note\b|\braw (?:event|participant|coalition)\b)/iu
  ],
  [
  ...
  [
    ...
    /\b(?:(?:was|is\s+being)\s+evicted|fac(?:e|es|ed|ing)\s+(?:eviction|foreclosure)|at\s+risk\s+of\s+eviction|received\s+an?\s+eviction\s+notice|landlord\s+filed\s+for\s+eviction|pending\s+foreclosure|lost\s+(?:her|his|their)\s+(?:apartment|lease)|forced\s+from\s+(?:her|his|their)\s+home|(?:became|is|was)\s+homeless|unhoused|(?:sleeps?|staying)\s+in\s+(?:an?\s+)?shelter|moved\s+into\s+(?:an?\s+)?shelter|entered\s+(?:emergency\s+housing\s+following|transitional\s+housing\s+after)\s+an?\s+eviction|los(?:e|t|ing)\s+housing|facing\s+housing\s+loss|housing\s+(?:instability|loss|lost)|behind\s+on\s+rent\s+and\s+may\s+lose\s+(?:her|his|their)\s+home|hiv(?:\s+positive)?|(?:hiv|cancer|disability)\s+diagnos(?:is|es)|(?:has|had)\s+(?:an?\s+)?disability|(?:has|had|treating)\s+cancer|living\s+with\s+cancer|diagnosed\s+with\s+leukemia|medical\s+conditions?|(?:being\s+)?treated\s+for\s+leukemia|medically\s+treated|(?:receives?|receiving)\s+psychiatric\s+treatment|(?:underwent|undergoing|receiving)\s+chemo(?:therapy)?(?:\s+treatments?)?|chemotherap(?:y|ies)|(?:undergoing|receiving)\s+radiation\s+therapy|(?:received|receiving|underwent|undergoing|medical)\s+treatments?|treatments?\s+for\s+(?:cancer|hiv|leukemia)|(?:receives?|receiving|needs?)\s+dialysis|(?:takes?|taking|receives?|receiving)\s+insulin|(?:takes?|taking)\s+antiretroviral\s+medication|diabet(?:es|ic)|disclos(?:ed|ing)\s+(?:an?\s+)?(?:hiv|pregnancy|disability\s+diagnosis)|hospitalized\s+for\s+depression|hospitalization|has\s+bipolar\s+disorder|uses?\s+(?:an?\s+)?wheelchair\s+due\s+to\s+(?:an?\s+)?injury|(?:in\s+recovery\s+from\s+substance|recovering\s+from\s+opioid)\s+use|medical\s+history|eviction\s+history)\b/i
  ],
  [
    "sensitive personal treatment narrative",
    /\b(?:resident|patient)(?:\s+\S+){0,4}\s+discuss(?:ed|ing)\s+treatments?\b/i
  ],
  [
    "local machine path",
    /(?:\/(?:Users|Volumes|Library|etc|home|mnt|opt|root|srv|tmp|workspace|private\/tmp|var\/(?:folders|db\/private))\/|~\/(?:Library|\.ssh|private|secrets?|sessions?|captures?|exports?)\/|\.\.?\/(?:private|secrets?|sessions?|captures?|exports?)\/|(?:^|[\s"'=])(?:private|secrets?|sessions?|captures?|exports?)\/[^\s?#]+|[A-Z]:\\(?:Users|Profiles|Documents and Settings)\\)/i
  ],
...
const meetingAccessHostPattern =
  /(?:^|\.)(?:zoom\.us|zoomgov\.com|meet\.google\.com|teams\.microsoft\.com|webex\.com|meet\.jit\.si|whereby\.com)$/i;
const meetingAccessUrlPattern =
  /(?:https?:\/\/)?(?:[^/]+\.)?(?:zoom\.us|zoomgov\.com|meet\.google\.com|teams\.(?:microsoft\.com|live\.com|cloud\.microsoft)|webex\.com|meet\.jit\.si|whereby\.com)\b/i;
...
export function validateNycacFacebookEventsManifest(
  ...
  if (manifest?.currentReplay?.performedAt !== "2026-07-16") {
    failures.push("manifest.currentReplay.performedAt must be 2026-07-16");
  }
  if (
    JSON.stringify(manifest?.currentReplay?.growthSequence) !==
    JSON.stringify([8, 32, 33])
  ) {
    failures.push("manifest.currentReplay.growthSequence must be [8, 32, 33]");
  }
