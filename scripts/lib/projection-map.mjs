...
export function renderProjectionMap(input = knowledgeLifecycle) {
  const sections = input.proofSurfaceManifests.map((manifest) => [
    ...
    `**Destination type:** \`${manifest.destinationType}\``,
    ...(manifest.artifactPath ? [`**Artifact:** \`${manifest.artifactPath}\``] : []),
    ...
    "",
    ...(manifest.canonicalClaimIds.length
      ? manifest.canonicalClaimIds.map((id) => `- \`${id}\``)
      : ["- None"]),
    "",
    "### Exclusions",
    "",
