...
const intakeObservations = knowledgeBank.observations.filter(
  (observation) => observation.intakeId === intake?.id
);
...
check(
  ...
  intake?.kind === "analysis-note" &&
    ...
    intakeObservations.length === 14 &&
    intakeObservations.every(
      (observation) => observation.publicSafe && observation.intakeId === intake.id
    ) &&
    personalClaimIds.every((id) => claimById.has(id)) &&
