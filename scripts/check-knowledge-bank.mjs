...
const publicAppText = textFiles
  ...
  .map((file) => read(file))
  .join("\n");
const publicVisibleAppText = textFiles
  .filter((file) => {
    return (
      isAppText(file) &&
      !file.startsWith("apps/www/src/data/") &&
      !file.startsWith("apps/www/src/types/")
    );
  })
  .map((file) => read(file))
  .join("\n");

const proofDataPath = "apps/www/src/data/proofs.ts";
const workDataPath = "apps/www/src/data/work.ts";
const proofDataText = exists(proofDataPath) ? read(proofDataPath) : "";
const workDataText = exists(workDataPath) ? read(workDataPath) : "";
const allowedProofStatuses = new Set(["ready", "use-with-care", "open", "protected", "do-not-publish"]);
const proofIds = [...proofDataText.matchAll(/\bid:\s*"([^"]+)"/g)].map((match) => match[1]);
const duplicateProofIds = proofIds.filter((id, index) => proofIds.indexOf(id) !== index);

if (!proofDataText) {
  report("failure", "Missing structured proof data", proofDataPath);
}
if (duplicateProofIds.length) {
  report("failure", "Duplicate proof claim IDs", proofDataPath, [...new Set(duplicateProofIds)].join(", "));
}

const proofStatusById = new Map();
for (const match of proofDataText.matchAll(/\{\s*id:\s*"([^"]+)"[\s\S]*?status:\s*"([^"]+)"/g)) {
  const [, id, status] = match;
  proofStatusById.set(id, status);
  if (!allowedProofStatuses.has(status)) {
    report("failure", "Unsupported proof status", proofDataPath, `${id}: ${status}`);
  }
}

if (!proofDataText.includes("allowedSurfaces")) {
  report("failure", "Structured proof data is missing allowed surfaces", proofDataPath);
}
if (!proofDataText.includes("guardrail")) {
  report("failure", "Structured proof data is missing guardrails", proofDataPath);
}
if (!workDataText.includes("proofBankIds")) {
  report("failure", "Work data is missing proofBankIds", workDataPath);
}

const referencedProofIds = [
  ...workDataText.matchAll(/proofBankIds:\s*\[([^\]]*)\]/g)
].flatMap((match) => [...match[1].matchAll(/"([^"]+)"/g)].map((idMatch) => idMatch[1]));

for (const id of referencedProofIds) {
  if (!proofStatusById.has(id)) {
    report("failure", "Work item references unknown proofBankId", workDataPath, id);
  }
  if (["protected", "do-not-publish"].includes(proofStatusById.get(id))) {
    report("failure", "Work item references protected/do-not-publish proof", workDataPath, id);
  }
}

for (const id of proofIds) {
  if (publicVisibleAppText.includes(id)) {
    report("failure", "Claim ID appears in public app text", "", id);
  }
}
...
for (const [label, pattern] of publicAntiClaimPatterns) {
  if (pattern.test(publicVisibleAppText)) {
...
if (
  !packageJson.scripts?.["check:production"]?.includes("check-knowledge-bank.mjs") &&
  !packageJson.scripts?.["check:production"]?.includes("preflight:production")
) {
