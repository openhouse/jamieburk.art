...
const missionPatterns = [
  ...
  },
  {
    ...
    summary:
      "Posts linked a grassroots venue manual, a member-made tutorial, Allied Media Conference, peer DIY funding infrastructure, and an archived article about self-authored DIY documentation.",
    statusIds: [
      "590942060829663232",
      "592810776961916929",
      "596691623993581568",
...
const sourceLeads = [
  ...
  },
  {
    ...
    disposition: "archived-and-close-read",
    note:
      "Elise Granata's May 6, 2015 article argues that DIY communities need self-authored archives and connective infrastructure. It is field context surfaced by the account, not evidence that the article covered WOW List or that Jamie authored it."
  },
  {
...
export function buildCorpus(rawCaptureText) {
  ...
  const corpus = {
    ...
    acquisitionVerification: rawCapture.acquisitionVerification,
    population: {
    ...
    boundaries: [
      ...
      "The authenticated-session identity used to reach the public timeline is excluded from public artifacts.",
      "Historical destinations are evidence of past communication and are not presented as current guidance."
    ],
    missionPatterns,
    sourceLeads,
  ...
  const canonicalStatusIds = items
    .map((item) => statusId(item.canonicalUrl))
    .sort();
  const statusIdSetSha256 = sha256(
    `${canonicalStatusIds.join("\n")}\n`
  );
  assert.equal(
    corpus.acquisitionVerification.statusIdSetSha256,
    statusIdSetSha256
  );
  assert.equal(corpus.acquisitionVerification.passes.length, 3);
  assert.equal(
    new Set(
      corpus.acquisitionVerification.passes.map((pass) => pass.distinctStatusIds)
    ).size,
    1
  );
  assert(
    corpus.acquisitionVerification.passes.every(
      (pass) => pass.matchedProfileBaseline && pass.matchedCanonicalStatusIdSet
    )
  );
  for (const pass of corpus.acquisitionVerification.passes) {
    assert.deepEqual(pass.statusIds, canonicalStatusIds);
  }
