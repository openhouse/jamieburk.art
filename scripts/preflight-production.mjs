...
if (routeCheckBaseUrl) {
  try {
    execFileSync(process.execPath, ["scripts/check-routes.mjs", routeCheckBaseUrl], {
      cwd: repoRoot,
      stdio: "pipe"
    });
  } catch (error) {
    const output = `${error.stdout?.toString() ?? ""}${error.stderr?.toString() ?? ""}`.trim();
    addError(output || "Production route check failed.");
  }
}
...
console.log(
  `Production preflight passed for ${
    robotsPolicy === "index" ? "final indexing" : "quiet noindex soft launch"
  }.`
);
