...
export function evaluate(candidate) {
  ...
  const canonical = records.filter(
    (record) => record.kind === "canonical-reference"
  );
  const gaps = records.filter(
    (record) => record.kind === "public-coverage-gap"
  );
  const publicSources = records.filter(
    (record) => record.kind === "public-source-record"
  );
  ...
  check(
    lock.source_repository === "openhouse/jamie-burkart-public-record",
    "unexpected source repository"
  );
  check(
    /^[0-9a-f]{40}$/.test(lock.source_commit ?? ""),
    "source commit is not pinned"
  );
  check(
    sha256(manifestSource) === lock.source_manifest_sha256,
    "manifest digest does not match lock"
  );
  check(
    manifest.candidate_fingerprint === lock.source_candidate_fingerprint,
    "candidate fingerprint does not match lock"
  );
  check(
    manifest.record_count === records.length &&
      records.length === lock.record_count,
    "record counts drifted"
  );
  check(
    manifest.public_release_authorized === false &&
      lock.public_release_authorized === false,
    "public release was inferred"
  );
  check(
    manifest.private_source_dependency === false &&
      lock.private_runtime_dependency === false,
    "public build gained a private dependency"
  );
  check(
    lock.projection_state === "knowledge-wiki-reference-only",
    "portfolio projection was authorized"
  );
  ...
  check(
    new Set(records.map((record) => record.canonical_path)).size ===
      records.length,
    "duplicate canonical path"
  );
  check(
    records.every((record) => /^[0-9a-f]{64}$/.test(record.sha256 ?? "")),
    "record digest missing or invalid"
  );
  check(
    canonical.every((record) =>
      /^[0-9a-f]{40}$/.test(record.canonical_commit ?? "")
    ),
    "canonical statement reference is not pinned"
  );
  check(
    canonical.every(
      (record) =>
    ...
    "canonical statement body was reassigned"
  );
  check(
    canonical.every((record) =>
      /\/blob\/[0-9a-f]{40}\//.test(record.canonical_record_url ?? "")
    ),
    "canonical statement URL is not content-addressed"
  );
  check(
    gaps.every(
      (record) => record.publication_state === "human-review-required"
    ),
    "coverage gap was promoted"
  );
  check(
    /no-photo-pixels-or-identifiers; publication-review-required/.test(
      manifest.photo_policy ?? ""
    ),
    "photo boundary weakened"
  );
  check(
    !/\/(?:Users|Volumes|private|tmp)\//.test(manifestSource),
    "local path leaked into manifest"
  );
  check(
    !/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i.test(manifestSource),
    "email address leaked into manifest"
  );
  check(
    !/communication_count|message_count|raw_body|transcript_body|photo_uuid|coordinates/i.test(
      manifestSource
    ),
    "protected field leaked into manifest"
  );
  ...
  check(
    ...
    "Wiki page does not cite pinned source commit"
  );
  check(
    wikiSource.includes(lock.source_manifest_sha256),
    "Wiki page does not cite manifest digest"
  );
  check(
    /relations:\s*[\s\S]*?- type: related_to\s+target: index\.knowledge-wiki\.jamie-public-record-source-edition/.test(
      sourcePageSource
    ),
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (!result.passed) {
    ...
    console.log(
      `Public-record source edition validated: ${result.metrics.records} records, ` +
        `${result.metrics.canonicalReferences} canonical references, ` +
        `${result.metrics.publicCoverageGaps} governed gaps.`
    );
  }
}
