...
Return only JSON matching the supplied schema. Set readerId to ${config.reader.id}. A pass requires verdict=pass and decision=${config.passDecision}; a fail requires verdict=fail and decision=${config.failDecision}. Set actualPersonParticipated=false. Even for a pass, return constructive critique and the most important thing to validate in a real kickoff. Do not repeat the reader's name anywhere in the returned JSON. The boundary must state that this is a fictionalized public-page model simulation, that "the named person" did not participate, and that the result is not the named person's endorsement or budget authorization.
