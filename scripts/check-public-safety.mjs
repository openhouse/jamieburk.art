...
const scanTargets = [
  { path: "apps/www/src", mode: "app" },
  { path: "apps/www/public", mode: "public" },
  { path: "README.md", mode: "root" },
  { path: "AGENTS.md", mode: "root" },
  { path: "Dockerfile", mode: "root" },
  { path: ".env.example", mode: "root" },
  { path: "docs", mode: "docs" }
];
...
const productionBlockers = [
  ...
  { label: "unresolved public email", pattern: /Public email pending confirmation/gi },
...
async function collectTargetFiles(target) {
  ...
  try {
    ...
    if (entry.isFile()) return [{ filePath: absolutePath, mode: target.mode }];
  ...
  for (const entry of entries) {
    ...
    const childRelative = relative(childPath);

    if (entry.isDirectory()) {
      files.push(...(await collectTargetFiles({ path: childRelative, mode: target.mode })));
    } else if (entry.isFile()) {
      files.push({ filePath: childPath, mode: target.mode });
    }
  }
...
function checkResumePdf(failures) {
  ...
  const phoneApprovedForThisBranch = true;
  if (!phoneApprovedForThisBranch && /\b(?:\+?1[\s.-]?)?(?:\(?\d{3}\)?[\s.-]?)\d{3}[\s.-]?\d{4}\b/.test(text)) {
    failures.push(`${resumePath} contains a phone number without approval`);
  }

  const publicEmail = process.env.NEXT_PUBLIC_CONTACT_EMAIL?.trim();
  if (publicEmail && !text.includes(publicEmail)) {
    failures.push(`${resumePath} does not include the approved public email`);
  }
}

function checkWorkItems(failures) {
...
export async function runPublicSafetyCheck({ production = false } = {}) {
  const failures = [];
  const warnings = [];
  const files = (await Promise.all(scanTargets.map(collectTargetFiles))).flat();
