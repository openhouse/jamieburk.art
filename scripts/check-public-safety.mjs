...
function checkText(filePath, text) {
  ...
  if (publicSource && /TODO:\s*Jamie approval required/i.test(text)) {
    addBlocker(rel, "visible approval TODO remains in public app source");
  }
...
function checkPdf(filePath) {
  ...
  if (placeholderResumeHashes.has(hash)) {
    addBlocker(rel, "known placeholder PDF hash is still present");
  }

  const text = readPdfText(filePath);
  if (/Placeholder Resume PDF|Replace with approved current resume/i.test(text)) {
    addBlocker(rel, "placeholder resume PDF text is still present");
  }
