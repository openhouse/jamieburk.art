...
const scannerFile = path.join(repoRoot, "scripts/check-public-safety.mjs");
const shippedContentFiles = shippedTextFiles.filter((file) => file !== scannerFile);
...
for (const file of allFiles) {
  ...
  if (
    /(^|\/)(private|archive-private|raw|transcripts-private|client-private|legal-review)(\/|$)/i.test(
      rel
    ) ||
    /\.private\./i.test(rel)
  ) {
  ...
  if (/\.(key|pem)$/i.test(rel)) {
    addFailure(file, "key material must not be committed");
  }
}
...
for (const file of textFiles.filter((item) => item !== scannerFile)) {
  const content = readText(file);
...
if (!existsSync(proofPath)) {
  failures.push("apps/www/src/data/proofs.ts - structured proof data is missing");
} else {
  const proofSource = readText(proofPath);

  if (/\/Users\/|\/Volumes\/|Mobile Documents|supporting-materials|otter(?:\.ai|_ai)|\.docx|\.xlsx/i.test(proofSource)) {
    addFailure(proofPath, "proof claim source text contains a private path or private source marker");
  }

  const proofIds = new Set(
    [...proofSource.matchAll(/id:\s*"([^"]+)"/g)].map((match) => match[1])
  );
  ...
  for (const match of proofSource.matchAll(/\{\n\s+id:\s*"([^"]+)"[\s\S]*?\n\s+\}/g)) {
    ...
    const surfacesBlock = /surfaces:\s*\[([\s\S]*?)\]/.exec(block)?.[1] ?? "";
    const surfaces = [...surfacesBlock.matchAll(/"([^"]+)"/g)].map((item) => item[1]);

    if ((status === "pending" || status === "private") && surfaces.some((surface) => publicSurfaces.has(surface))) {
      addFailure(proofPath, `pending/private proof claim is projected publicly: ${id}`);
    }

    if (status === "ready" && /TODO|approval required/i.test(block)) {
      addFailure(proofPath, `ready proof claim contains unresolved approval language: ${id}`);
    }

    if (status === "careful") {
      addWarning(proofPath, `careful proof claim requires guardrail review: ${id}`);
    }

    if (/firsthand-collaborator-context/.test(block)) {
      addWarning(proofPath, `firsthand collaborator context needs careful projection: ${id}`);
    }
  }

  const workPath = path.join(repoRoot, "apps/www/src/data/work.ts");
  if (existsSync(workPath)) {
    const workSource = readText(workPath);
    for (const match of workSource.matchAll(/proofBankIds:\s*\[([\s\S]*?)\]/g)) {
      const ids = [...match[1].matchAll(/"([^"]+)"/g)].map((item) => item[1]);
      for (const id of ids) {
        if (!proofIds.has(id)) {
          addFailure(workPath, `work metadata references missing proof claim: ${id}`);
        }
      }
    }

    if (/visibility:\s*"private"/.test(workSource)) {
      addFailure(workPath, "private work item is present in public data");
    }

    if (/status:\s*"Draft"/.test(workSource)) {
      addFailure(workPath, "draft work item is present in public data");
    }
  }
}
