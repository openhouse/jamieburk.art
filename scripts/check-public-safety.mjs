...
const failures = [];
const warnings = [];
const sensitiveFileExtensions = new Set([".key", ".pem", ".p12", ".pfx", ".crt", ".cer"]);
...
for (const relativeFile of trackedFiles()) {
  ...
  if (sensitiveFileExtensions.has(ext)) {
    addFinding(failures, "tracked key/certificate file is not allowed", relativeFile);
  }
...
if (!existsSync(resumePath)) {
  ...
  if (bytes.length < 10_000) {
    addFinding(
      productionLike ? failures : warnings,
      "resume PDF is unexpectedly small",
      toRelative(resumePath)
    );
  }
  if (
  ...
  try {
    const extracted = execFileSync("pdftotext", [resumePath, "-"], {
      cwd: repoRoot,
      encoding: "utf8",
      maxBuffer: 1024 * 1024 * 4
    });
    if (!/Jamie Burkart/i.test(extracted)) {
      addFinding(
        productionLike ? failures : warnings,
        "resume PDF text does not include Jamie Burkart",
        toRelative(resumePath)
      );
    }
  } catch {
    addFinding(warnings, "pdftotext unavailable; resume text check skipped", toRelative(resumePath));
  }
}
