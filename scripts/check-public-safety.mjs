...
for (const file of trackedFiles) {
  ...
  if (privatePathPatterns.some((pattern) => file === pattern.slice(0, -1) || file.startsWith(pattern))) {
  ...
  if (/\.(ttf|otf|woff|woff2)$/i.test(file)) {
    hardFailures.push(`Tracked font binary requires explicit license approval: ${file}`);
  }
}
...
const productionFacingFiles = trackedFiles.filter((file) => {
  if (file === "apps/www/mdx-components.tsx" || file === "apps/www/next.config.ts") {
    return true;
  }

  return (
    (file.startsWith("apps/www/src/") || file.startsWith("apps/www/public/")) &&
    (textExtensions.has(path.extname(file)) || path.extname(file).toLowerCase() === ".pdf")
  );
});
...
const hardTextChecks = [
  ...
  },
  {
    ...
    pattern: /\b(?:AWS_SECRET_ACCESS_KEY|OPENAI_API_KEY|api_key|password|secret)\s*=/i
  },
  {
    label: "private local source path",
    pattern: /(?:\/Volumes\/16TB_SSD\/Work\/Jamie\/Portfolio\/supporting-materials|\/Users\/jburkart\/Library\/Mobile Documents|Jamie Projects History|job-hunt)/i
  }
];
...
const productionBlockerChecks = [
  ...
  },
  {
    ...
    pattern: /(?:pending approval|approved screenshot pending|pending screenshot|pending citation|unapproved contact|unapproved metric)/i
  },
  {
    label: "raw transcript marker",
    pattern: /(?:otter\.ai|raw transcript|^\s*[A-Z][A-Za-z .'-]{1,60}\s+\d{1,2}:\d{2}(?::\d{2})?\s*$)/im
  }
];
...
for (const file of productionFacingFiles) {
  ...
  for (const check of productionBlockerChecks) {
    const match = check.pattern.exec(text);
    if (match) {
      const message = `${file}:${lineNumberFor(text, match.index)} contains ${check.label}`;
      if (mode === "production") {
