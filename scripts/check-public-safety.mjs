...
function scanResumePdf() {
  ...
  const text = extractPdfText(absolute);

  if (/Placeholder resume PDF|Replace with approved current resume/i.test(text)) {
    addFailure(resumePath, 1, "resume PDF still contains placeholder text");
  }

  if (/home address|street address/i.test(text)) {
    addWarning(resumePath, 1, "review resume for address-like language");
  }
}
