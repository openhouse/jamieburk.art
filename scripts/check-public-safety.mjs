...
const searchableFiles = trackedFiles.filter((file) => {
  if (!scanFile(file)) return false;
  const ext = path.extname(file).toLowerCase();
  return textExtensions.has(ext) || ext === ".pdf" || ext === "";
});

const productionFacingFiles = searchableFiles.filter((file) => {
  return (
    file.startsWith("apps/www/src/") ||
    file.startsWith("apps/www/content/") ||
    file.startsWith("apps/www/public/") ||
...
const hardTextChecks = [
  ...
  },
  {
    ...
    pattern: /\b(?:AWS_SECRET_ACCESS_KEY|OPENAI_API_KEY|api_key|access_token|auth_token|password|secret)\s*=/i
  },
  {
