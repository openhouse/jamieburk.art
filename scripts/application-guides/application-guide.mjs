...
export function renderGuide(config, sources) {
  const sections = [
    ...
    `[Official posting and Apply button](${config.postingUrl}). The posting was checked on ${config.reviewedOn} and lists **${config.deadline}** as its closing date. Recheck it before submitting. Confirm **${config.role}**, **OTI**, **Brooklyn**, and **Job ID ${config.jobId}**; this is not Senior Product Manager 782366.`,
  ...
  sections.push(
    ...
    ...renderFields(config.fields.filter((field) => profileIds.has(field.id))),
    '## 7. Resume upload',
    `Choose **${config.sources.pdf.path}** in the required **Resume** field. Wait for the filename/upload confirmation. Reopen the selected PDF and verify that it is the Product Manager version, not the earlier Senior Product Manager application.`,
    ...
    'Field labels and the 10 MB limit come from Jamie-provided SmartRecruiters screenshots. The expanded Experience/Education controls are carried forward from the earlier application on the same platform, not presented as newly inspected 784450 controls. No later-page coverage or successful live submission is claimed.',
