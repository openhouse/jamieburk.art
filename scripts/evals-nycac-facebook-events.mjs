...
const populationIntake = knowledgeBank.intake.find(
  (item) => item.id === "INT-NYCAC-FACEBOOK-EVENTS-FULL-POPULATION-2026"
);
const memoryIntake = knowledgeBank.intake.find(
  (item) => item.id === "INT-NYCAC-FACEBOOK-EVENT-PRACTICE-MEMORY-2026"
);
...
const additionalUnsafeMutations = [
  ...
  (candidate) => {
    candidate.events[0].title =
      `Token ${["xo", "xb-123456789012-123456789012-abcdefghijklmnopqrstuv"].join("")}`;
  },
  (candidate) => {
    candidate.events[0].title =
      "Token github_pat_11AA00_exampleexampleexample";
  },
  (candidate) => {
  ...
  ...[
    ...
    "Call 212\u2013555\u20131212 for details",
    ["xo", "xc-123456789012-abcdefghijklmnopqrstuvwxyz"].join(""),
    ["xo", "xe.xoxp-1-abcdefghijklmnopqrstuvwxyz"].join(""),
    "AIzaSyDUMMYDUMMYDUMMYDUMMYDUMMY12345",
  ...
  ...[
    ...
    `https://nycartc.com/public?token=${encodeURIComponent(
