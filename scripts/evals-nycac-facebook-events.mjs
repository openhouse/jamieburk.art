...
const idDigest = createHash("sha256").update([...ids].sort().join("\n")).digest("hex");
const fixtureSha = createHash("sha256").update(fixtureText).digest("hex");

check(
  "Population reconciliation",
  "All displayed control slots have an honest recovered-or-unresolved disposition",
  15,
  fixture.page.handle === "nycartc" &&
...
check(
  "Population reconciliation",
  "Platform volatility is preserved without erasing earlier recovered records",
  7,
...
check(
  ...
  JSON.stringify(years) ===
    ...
    includesAll(normalizedDocs, [
      ...
      "not identify the individual author or producer of every page or program"
    ])
);
...
check(
  ...
  "The recurring rotating-space pattern and its limits are explicit",
  12,
  recurring.length === 12 &&
...
for (const event of events) {
  for (const topic of event.topics) topicCounts[topic] = (topicCounts[topic] ?? 0) + 1;
}
...
const articlePublishers = fixture.postedSourceArticles.map((article) => article.publisher);
...
check(
  ...
  9,
  fixture.postedSourceArticles.length === 7 &&
...
const populationLead = knowledgeLifecycle.leads.find(
  (item) => item.id === "LEAD-NYCAC-FACEBOOK-EVENT-FULL-POPULATION"
);
...
check(
  ...
  "The pass reaches leads, observations, candidates, decisions, canonical claims, and inquiries",
  10,
  populationLead?.sourceIds.length === 3 &&
    populationLead.candidateClaimIds.length === 4 &&
    roleLead?.sourceIds.length === 4 &&
    lifecycleObservations.length === 9 &&
    lifecycleCandidates.length === 4 &&
    lifecycleCandidates.every((candidate) => candidate.maturity === "promoted") &&
    lifecycleEvents.length === 4 &&
    lifecycleTasks.length === 2 &&
    lifecycleDecisions.length === 4 &&
    lifecycleDecisions.every(
      (decision) =>
        decision.decision === "promote" &&
        decision.reviewAuthority === "jamie-approved" &&
...
const proofBlock = proofSource.slice(
  proofSource.indexOf('id: "nyc-artist-coalition-participation-system"'),
  proofSource.indexOf('id: "wowlist-community-platform"')
);

check(
  "Role and projection discipline",
  "The website receives the strong bounded role claim while fragile metrics stay in the bank",
  9,
  participationClaim?.projections.some(
    ...
    includesAll(caseStudy, [
      ...
      "does not assign Jamie authorship of every event",
...
check(
  "Public safety",
  "The metadata-only fixture is hash-locked and excludes personal, raw, and authenticated data",
  8,
  fixtureSha ===
    "64af7b2f1804b3b319de2f5eef60bfb01371ce5209c8497473f800a334c66555" &&
    fixture.publicSafety.rawDescriptionsPublished === false &&
    fixture.publicSafety.attendeeIdentitiesPublished === false &&
    fixture.publicSafety.contactDetailsPublished === false &&
    fixture.publicSafety.accessCredentialsPublished === false &&
    !objectKeys.some((key) =>
      [
        "rawDescription",
        "attendeeIdentities",
...
console.log(
  `NYCAC Facebook events eval: ${score}/100 (criterion: >= ${threshold}, no hard failures)`
);
