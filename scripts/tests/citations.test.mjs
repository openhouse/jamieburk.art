...
test("corrections retire old wording from public surfaces", () => {
  const text = ["apps/www/src/content/work/callnyc.mdx", "apps/www/src/content/work/fair-rent-nyc.mdx", "apps/www/src/data/work.ts", "apps/www/src/data/proofs.ts", "apps/www/src/app/resume/page.tsx"].map((path) => readFileSync(path, "utf8")).join("\n");
  assert.doesNotMatch(text, /first civic-data hackathon|2014[-–]2015|30\+ pages of shared campaign-memory/i);

  const correctionIds = new Set(knowledgeBank.corrections.map((item) => item.id));
  for (const id of [
    "COR-CALLNYC-CHRONOLOGY-2026",
    "COR-CALLNYC-SUPERLATIVE-2026",
    "COR-CALLNYC-EVENT-TIME-2026",
    "COR-FAIR-RENT-MEMORY-PAGE-COUNT-2026"
  ]) {
    assert.ok(correctionIds.has(id), `Missing required correction ${id}`);
  }
});
