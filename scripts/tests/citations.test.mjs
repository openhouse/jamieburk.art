...
test("canonical registry passes deterministic validation", () =>
  assert.deepEqual(validateKnowledgeBank(), []));

test("page-local numbering follows first source appearance", () => {
  assert.deepEqual(
    resolveCitationOccurrence("callnyc", "event-date-time").sources.map(
      (item) => item.number,
    ),
    [1, 2],
  );
  assert.deepEqual(
    ...
    ).sources.map((item) => item.number),
    [2],
  );
  assert.deepEqual(
    resolveCitationOccurrence("callnyc", "independent-follow-on").sources.map(
      (item) => item.number,
    ),
    [3, 4],
  );
  assert.deepEqual(
    resolveCitationOccurrence("callnyc", "event-branding").sources.map(
      (item) => item.number,
    ),
    [5],
  );
...
test("multi-source occurrences preserve editorial order", () => {
  assert.deepEqual(
    ...
    ["SRC-CALLNYC-POLITICO-2016-03-14", "SRC-CALLNYC-GITHUB-REPOSITORY"],
  );
});
...
test("negative research preserves scope and limitations", () => {
  ...
  assert.ok(
    inquiry.limitations.some((item) => /not proof of nonexistence/i.test(item)),
  );
