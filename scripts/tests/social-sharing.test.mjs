...
const [
  { createMetadata },
  { portfolioPhotos },
  { homeSocialCard, resolveSocialCardPhotoPath }
] = await Promise.all([
...
test("Open Graph and Twitter expose the same photographic large-card contract", () => {
  ...
  assert.equal(
    ...
    "https://staging.jamieburk.art/opengraph-image?v=human-index-photo-v1"
  );
  ...
  assert.equal(homeSocialCard.revision, "human-index-photo-v1");
});
