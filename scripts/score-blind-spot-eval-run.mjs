...
export function scoreRun(suite, run) {
  ...
  if (typeof run.candidate_sha !== "string" || !run.candidate_sha.trim()) errors.push("candidate_sha is required");
  if (typeof run.rubric_sha !== "string" || !run.rubric_sha.trim()) errors.push("rubric_sha is required");
  if (!Array.isArray(run.content_scope) || !run.content_scope.length) errors.push("content_scope is required");
  ...
  for (const [index, result] of (run.results ?? []).entries()) {
    ...
    if (result.recommended_next_move !== null && typeof result.recommended_next_move !== "string") {
  ...
  for (const entry of suite.evals) {
    ...
    if (entry.blocking && result.pass !== true) blockers.push(`${entry.id} blocking eval did not pass`);
  }
  ...
  if (!Number.isInteger(run.consecutive_passing_runs) || run.consecutive_passing_runs < 2) {
    blockers.push("two consecutive passing runs are required");
  }

  if (run.target === "release-readiness") {
    for (const field of [
      "independent_holdout_pass",
      "cold_reader_validation_pass",
      "live_use_validation_pass"
    ]) {
      if (run[field] !== true) blockers.push(`${field} is required`);
    }
...
function runCli() {
  ...
  if (!runPath) {
    console.error("Usage: npm run evals:blind-spots:score -- <run.json>");
    process.exit(2);
  }
