...
function authoredSourcePostBody(item) {
  if (item.kind !== "authored" || !item.text) return "";

  const sourceCard = item.text.split("\nQuote\n", 1)[0];
  const lines = sourceCard.split("\n");
  const accountHandleIndex = lines.findIndex(
    (line) => line.trim().toLowerCase() === "@nycartc"
  );
  const body = accountHandleIndex >= 0 ? lines.slice(accountHandleIndex + 1) : lines;

  while (
    body.length > 0 &&
    (body[0].trim() === "" ||
      body[0].trim() === "·" ||
      /^[A-Z][a-z]{2} \d{1,2}, \d{4}$/.test(body[0].trim()))
  ) {
    body.shift();
  }
...
export function buildNycArtCCorpus(rawCaptureText) {
  ...
  const normalizeItem = (item) => {
    const classificationInput = authoredSourcePostBody(item);
    return {
      ...
      classificationInputSha256: sha256(classificationInput),
      publicTextOmitted: item.publicTextOmitted,
      hashtags: item.hashtags,
      mentions: item.mentions,
      visibleHashtags: visibleTokens(classificationInput, "#"),
      visibleMentions: visibleTokens(classificationInput, "@"),
  ...
  for (const statusId of ["1995868766614462973", "1674013523373068289"]) {
    assert.equal(
      items.find((item) => item.statusId === statusId)?.kind,
      "authored",
      `${statusId} must retain source-status authorship despite a self-repost appearance`
    );
  }
  ...
  const corpus = {
    ...
    heldObservations: {
      ...
      authoredPostsWithDisplayedReplyRepostOrLike: authored.filter(
        (item) =>
          item.visibleInteractions.replies > 0 ||
          item.visibleInteractions.reposts > 0 ||
          item.visibleInteractions.likes > 0
      ).length,
      status: "hold",
      reason:
        "Displayed reply, repost, and like labels are volatile, incomplete platform observations. Bookmarks and views are excluded from the qualifying-record count. All counters are retained for future comparison and are not projected as accomplishment metrics."
    },
  ...
  assert.deepEqual(deriveNycArtCCorpusMetrics(corpus), {
    ...
    nycCouncilOutboundMentions: 104,
    nycCouncilOutboundPosts: 100,
    olympiaKaziRecoveredReposts: 194,
    authoredPostsWithDisplayedReplyRepostOrLike: 628,
    visibleInteractionTotals: {
      replies: 112,
      reposts: 1_527,
      likes: 2_761,
  ...
  assert.equal(authoredHashtags.filter((tag) => tag === "#fairrentnyc").length, 195);
  return corpus;
}
...
export function deriveNycArtCCorpusMetrics(corpus) {
  ...
  return {
    ...
    authoredPostsWithDisplayedReplyRepostOrLike:
      corpus.heldObservations.authoredPostsWithDisplayedReplyRepostOrLike,
