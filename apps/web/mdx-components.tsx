...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    h2: ({ children }) => (
      <h2 className="jb-display mt-12 text-2xl font-semibold text-jb-ink">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="jb-display mt-8 text-xl font-semibold text-jb-ink">{children}</h3>
    ),
