...
export function AtAGlance({ item }: { item: WorkMeta }) {
  ...
  return (
    <section aria-labelledby="at-a-glance" className="rounded-lg bg-jb-blue p-5 text-jb-paper">
      <h2 className="jb-display text-xl font-semibold" id="at-a-glance">
        At a glance
      </h2>
      <dl className="mt-5 grid gap-4 sm:grid-cols-2">
        {rows.map(([label, value]) => (
          <div key={label}>
            <dt className="jb-display text-xs font-semibold uppercase text-jb-paper/70">
              {label}
            </dt>
      ...
      <div className="mt-5">
        <p className="jb-display text-xs font-semibold uppercase text-jb-paper/70">
          Tags
        </p>
        <div className="mt-3">
...
export function ArtifactList({ item }: { item: WorkMeta }) {
  return (
    <section aria-labelledby="artifact-list">
      <h2 className="jb-display text-2xl font-semibold text-jb-ink" id="artifact-list">
        Primary artifacts
      </h2>
      <ul className="mt-4 grid gap-3 sm:grid-cols-2">
        {item.artifactTypes.map((artifact) => (
          <li
            className="jb-display rounded-lg border border-jb-ink/12 bg-jb-paper px-4 py-3 text-sm font-medium text-jb-ink"
            key={artifact}
          >
            {artifact}
          </li>
...
export function ArtifactGallery({ item }: { item: WorkMeta }) {
  return (
    <section aria-labelledby="artifact-gallery">
      <h2 className="jb-display text-2xl font-semibold text-jb-ink" id="artifact-gallery">
        Artifact gallery
      </h2>
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        {item.artifacts.map((artifact, index) => (
          <JBCard className="jb-artifact-surface min-h-56" key={artifact.title}>
            <p className="jb-display text-xs font-semibold uppercase text-jb-blue">
              {artifact.type} / 0{index + 1}
            </p>
            <h3 className="jb-display mt-10 text-xl font-semibold text-jb-ink">{artifact.title}</h3>
...
export function KnownOpenProtected({ item }: { item: WorkMeta }) {
  ...
  return (
    <section aria-labelledby="known-open-protected">
      <h2 className="jb-display text-2xl font-semibold text-jb-ink" id="known-open-protected">
        Known / Open / Protected
      </h2>
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        {blocks.map(([label, text]) => (
          <JBCard key={label}>
            <h3 className="jb-display text-lg font-semibold text-jb-blue">{label}</h3>
...
function NoteBlock({
  ...
  return (
    <section className={`rounded-lg border p-5 ${styles}`}>
      <h2 className="jb-display text-xl font-semibold text-jb-ink">{title}</h2>
...
export function CreditsList({ item }: { item: WorkMeta }) {
  ...
  return (
    <section aria-labelledby="credits-list">
      <h2 className="jb-display text-2xl font-semibold text-jb-ink" id="credits-list">
        Credits
      </h2>
