...
function preferredUrl(source: SourceRecord) {
  ...
  return (
    <li id={noteId}>
      <p>
        ...
        {primaryUrl ? (
          <a href={primaryUrl}>{source.publicCitation}</a>
        ) : (
          <span>{source.publicCitation}</span>
        )}
      </p>
      ...
      {source.doesNotEstablish?.length ? (
        <p className="jb-endnote-boundary">
          <strong>Boundary:</strong> This source does not establish{" "}
          {source.doesNotEstablish.join(", ")}.
        </p>
      ) : null}
