...
export function WorkCard({ item }: WorkCardProps) {
  return (
    <article className="min-w-0 rounded-lg border border-jb-ink/15 bg-jb-warm/88 p-5 shadow-sm">
      <div className="flex flex-col gap-4">
        ...
        <div>
          <p className="jb-meta-label text-sm text-jb-ink">Role fit</p>
          <p className="mt-1 text-sm leading-6 text-jb-ink/72">{item.roleFit}</p>
        </div>
        <dl className="grid gap-3 text-sm md:grid-cols-2">
          <div>
            <dt className="jb-meta-label text-jb-ink">What was unclear</dt>
            <dd className="mt-1 leading-6 text-jb-ink/72">{item.whatWasUnclear}</dd>
          </div>
          <div>
            <dt className="jb-meta-label text-jb-ink">What became usable</dt>
