...
const campaignPressArticleSeeds = [
  ...
  {
    sourceId: "SRC-PRESS-SAVE-BEDFORD-BOWERY-NIGHT-MAYOR-2017",
    ...
    archiveTimestamp: "20260106102010"
  },
  {
...
export const campaignPressSources = [
  ...
  ...campaignPressArticleSeeds.map((seed): SourceRecord => ({
    ...
    publicNote: `The ${campaignPressCampaigns[seed.campaignId].title} campaign site's Press section selected this article. The article remains a metadata-level source lead until close reading.`,
    supportsGenerally: [
      "article identity, title, publisher, and a retrievable archive for future close reading"
    ],
    doesNotEstablish: [
...
export const campaignPressManifests = [
  ...
  },
  {
    ...
    articleSourceIds: [
      ...
      "SRC-PRESS-SAVE-BEDFORD-BOWERY-NIGHT-MAYOR-2017",
...
export const campaignPressExistingSourceArchiveUrls = {
  "SRC-NYCAC-GOTHAMIST-CABARET-2017-06-19":
    "https://web.archive.org/web/20190507132352/http://gothamist.com/2017/06/19/cabaret_law_nyc.php",
  "SRC-NYCAC-NPR-KUAF-CABARET-2017-09-20":
    "https://web.archive.org/web/20251028172606/https://www.npr.org/sections/therecord/2017/09/20/552292586/with-its-no-dancing-law-verging-on-repeal-new-york-legitimizes-its-nightlife",
  "SRC-NYCAC-VICE-DIY-SAFETY-2017-03-21":
    "https://web.archive.org/web/20170322022601/https://thump.vice.com/en_us/article/nyc-artist-coalition-dance-liberation-network-diy-spaces"
} as const;

export function campaignPressArchiveUrlFor(sourceId: string) {
  const source = campaignPressSources.find((item) => item.id === sourceId);
  return source?.archiveUrl ??
    campaignPressExistingSourceArchiveUrls[
      sourceId as keyof typeof campaignPressExistingSourceArchiveUrls
    ];
}

const pressClaimId = "CLM-NYCAC-CAMPAIGN-PRESS-CORPUS";
const pressResearchTaskId = "RT-NYCAC-CAMPAIGN-PRESS-CLOSE-READING";
const pressResearchInquiryId = "INQ-NYCAC-CAMPAIGN-PRESS-CORPUS";

const indexObservationId = (campaignId: CampaignPressCampaignId) =>
  `OBS-NYCAC-PRESS-INDEX-${campaignId.toUpperCase()}`;

const articleObservationId = (sourceId: string) =>
  `OBS-PRESS-METADATA-${sourceId.slice(4)}`;

export const campaignPressObservations = [
  ...campaignPressManifests.map((manifest): ObservationRecord => {
    const campaign = campaignPressCampaigns[manifest.campaignId];
    return {
      id: indexObservationId(manifest.campaignId),
      sourceId: manifest.indexSourceId,
      project: "nyc-artist-coalition",
      statement: `The captured ${campaign.title} Press section contains ${manifest.articleSourceIds.length} article placements in the preserved source-page order.`,
      observationType: "explicit",
      locator: "Press section; responsive duplicate markup collapsed before counting.",
      confidence: "high",
      limitations: [
        "This establishes campaign selection and placement count, not the article bodies, Jamie's individual role, or every article ever published about the campaign."
      ],
      supportsClaimIds: [pressClaimId],
      reviewedAt: "2026-07-14"
    };
  }),
  ...campaignPressArticleSeeds.map((seed): ObservationRecord => ({
    id: articleObservationId(seed.sourceId),
    sourceId: seed.sourceId,
    project: "nyc-artist-coalition",
    statement: `The archived publisher page identifies “${seed.title}” as an article from ${seed.organization}.`,
    observationType: "metadata",
    locator: "Publisher title and publication metadata preserved in the recorded Wayback capture.",
    confidence: "high",
    limitations: [
      "This metadata observation does not establish the article's propositions, Jamie's role, campaign causality, or endorsement of the article."
    ],
    supportsClaimIds: [],
    reviewedAt: "2026-07-14"
  }))
...
export const campaignPressClaims = [
  {
    ...
    observationIds: campaignPressManifests.map((manifest) =>
      indexObservationId(manifest.campaignId)
    ),
    projections: [
      {
        key: "archive-note",
    ...
    researchTaskIds: [pressResearchTaskId],
    researchInquiryIds: [pressResearchInquiryId],
    reviewedAt: "2026-07-14",
    reviewedBy: ["Jamie Burkart", "Codex public-source review"]
  }
] satisfies ClaimRecord[];

export const campaignPressCaptures = campaignPressManifests.map(
  (manifest): CaptureRecord => {
    const campaign = campaignPressCampaigns[manifest.campaignId];
    return {
      id: `CAP-${manifest.campaignId.toUpperCase()}-PRESS-CORPUS-2026-07-14`,
      receivedAt: "2026-07-14",
      submittedBy: "Jamie Burkart and Codex public-source review",
      kind: "url",
      summary: `Inventory of the ${manifest.articleSourceIds.length} article placements in the ${campaign.title} Press section, preserving source-page order, normalized article identity, and archive paths.`,
      sourceUrl: campaignPressSources.find((source) => source.id === manifest.indexSourceId)?.archiveUrl,
      status: "researching",
      publicSafety: "public-safe",
      potentialProjectIds: [...campaign.projectIds],
      potentialClaimFamilies: ["campaign press corpus", "historical context", "future claim research"],
      sourceIds: [manifest.indexSourceId, ...manifest.articleSourceIds],
      observationIds: [indexObservationId(manifest.campaignId)],
      researchTaskIds: [pressResearchTaskId],
      disposition: "The complete Press-section manifest is integrated. Article bodies remain routed to bounded close reading; no personal or causal claim was inferred from campaign selection or headline metadata."
    };
  }
);

export const campaignPressResearchTasks = [
  {
    id: pressResearchTaskId,
    project: "nyc-artist-coalition",
    question: "Which article-level propositions, quotations, collaborator credits, chronology, and outcomes can be responsibly developed from the campaign press corpus?",
    priority: "high",
    status: "open",
    captureIds: campaignPressCaptures.map((capture) => capture.id),
    sourceIds: campaignPressArticleSourceIds,
    claimIds: [pressClaimId],
    successCriteria: [
      "Close-read selected articles before attaching them to role, outcome, scale, or causal claims.",
      "Record atomic observations with locators, limitations, and collaborator or institutional credit.",
      "Triangulate any claim about Jamie's role or policy causality before considering public projection."
    ],
    nextActions: [
      "Prioritize sources that name Jamie, identify coalition collaborators, or document a concrete campaign action or public consequence.",
      "Preserve contradictions and negative findings instead of promoting headline-level inference.",
      "Return matured claims to the purpose-specific promotion slate; keep the full bibliography in the bank."
    ],
    publicNote: "The article corpus is preserved for selective proposition-level research; campaign selection alone supports no personal accomplishment claim.",
...
export const campaignPressInquiries = [
  {
    ...
    findings: [
      ...
      "Three articles were already canonical sources for bounded NYC Artist Coalition claims; 41 additional article records are now preserved for claim-by-claim close reading."
    ],
    limitations: [
