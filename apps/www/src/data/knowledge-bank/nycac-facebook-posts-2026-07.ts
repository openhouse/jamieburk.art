...
const sourceIds = {
  ...
  protectedRun: "SRC-NYCAC-FACEBOOK-PROTECTED-RUN-2026",
...
export const nycacFacebookPostKnowledge = {
  intakeItems: [
    {
      ...
      sourceIds: [...Object.values(sourceIds), ...existingGovernedSourceIds],
      observationIds: [
  ...
  sources: [
    ...
    },
    {
      ...
      title: "Authenticated NYC Artist Coalition Facebook post research capture",
      author: "Codex authenticated archival review",
      kind: "research-run",
      visibility: "protected",
      preservationStatus: "private",
      capturedAt: reviewedAt,
      publicCitation: "Authenticated archival-production review of the NYC Artist Coalition Facebook post surface, July 15, 2026.",
      ...
      protectedLocatorId: "LOC-NYCAC-FACEBOOK-POST-RESEARCH-2026",
      supportsGenerally: [
        "598 encountered render rows before reconciliation",
        "seven terminal checks with the January 2017 endpoint preserved",
  ...
  claims: [
    {
      ...
      projections: [
        { key: "case-study", text: "A complete capture-date pass of the surviving NYC Artist Coalition Facebook feed preserves 445 dated posts from 2017-2021, with public-safe source routing and shared-account authorship boundaries.", status: "hold", citationRequired: true, surfaces: [] },
        { key: "archive-note", text: "The public-safe census reconciles 598 encountered render rows into 445 distinct dated posts spanning 2017-2021.", status: "hold", citationRequired: true, surfaces: [] }
      ],
      evidence: [
    ...
    },
    {
      ...
      status: "confirmed-with-boundary",
      projections: [
        { key: "archive-note", text: "Across overlapping campaign arcs, the recovered Page repeatedly connected cultural-space concerns with public meetings, source articles, government interfaces, practical resources, and civic action routes.", status: "hold", citationRequired: true, surfaces: [] }
      ],
      evidence: [
    ...
    },
    {
      ...
      status: "confirmed-with-boundary",
      projections: [
        { key: "archive-note", text: "At capture time, Facebook displayed 2,291 reactions and 212 comments across the recovered population; these volatile labels are not unique reach, attendance, endorsement, conversion, mandate, or impact.", status: "hold", citationRequired: true, surfaces: [] }
      ],
      evidence: [
...
const legacyIntake = nycacFacebookPostKnowledge.intakeItems[0];

export const nycacFacebookPostIntakeItems: KnowledgeBank["intakeItems"] = [
  {
    id: "INTAKE-2026-07-15-NYCAC-FACEBOOK-POST-FULL-POPULATION",
    receivedAt: reviewedAt,
    inputKind: "metric",
    summary: legacyIntake.reason,
    projectIds: legacyIntake.projectIds,
    researchStatus: "researched",
    publicationStatus: "knowledge-bank-only",
    sourceIds: [...legacyIntake.sourceIds],
    observationIds: [...legacyIntake.observationIds],
    claimIds: Object.values(nycacFacebookPostClaimIds),
    researchInquiryIds: [...legacyIntake.researchInquiryIds],
    nextActions: [
      "Request a native Meta Page-owner export and reconcile post identities, visibility states, publishing metadata, and unexposed records against the 445-row ledger.",
      "Invite Olympia Kazi and other collaborators to distinguish account creation, identity design, publishing, moderation, campaign work, and period-specific stewardship.",
      "Resolve and preserve the 56 inventory-only routes without treating not rechecked as dead, live, or historically nonexistent.",
      "Research incoming engagement by key stakeholder groups from lawful public-safe records rather than inferring it from Page-authored references."
    ]
  }
];

export const nycacFacebookPostObservations: KnowledgeBank["observations"] =
  nycacFacebookPostKnowledge.observations.map((item) => ({
    id: item.id,
    sourceId: item.sourceId,
    project: item.project,
    text: item.text,
    locator: item.locator,
    status: item.status === "captured" ? "provisional" : "verified",
    confidence: item.status === "captured" ? "moderate" : "high",
    claimIds: [...item.claimIds],
    researchInquiryIds: [...item.researchInquiryIds],
    reviewedAt,
    reviewedBy:
...
export const nycacFacebookPostSources =
  nycacFacebookPostKnowledge.sources as KnowledgeBank["sources"];

export const nycacFacebookPostClaims =
  nycacFacebookPostKnowledge.claims as KnowledgeBank["claims"];

export const nycacFacebookPostResearchInquiries =
  nycacFacebookPostKnowledge.researchInquiries as KnowledgeBank["researchInquiries"];
