...
export const socialMediaArchiveProduction = {
  inventory: {
    ...
    accounts: [
      { project: "callnyc", handle: "@CallNYCapp", profilePosts: 110, recoveredStatuses: 86, recoveredAuthoredPosts: 72 },
      { project: "nyc-artist-coalition", handle: "@NYCArtC", profilePosts: 5124, recoveredStatuses: null, recoveredAuthoredPosts: null },
      { project: "wowlist", handle: "@wowlist", profilePosts: 38, recoveredStatuses: 37, recoveredAuthoredPosts: 21 },
      { project: "kc-spaces-fund", handle: "@KCSpacesFund", profilePosts: 35, recoveredStatuses: 34, recoveredAuthoredPosts: 27 },
      { project: "kc-town-hall", handle: "@KCTownHall", profilePosts: 183, recoveredStatuses: 170, recoveredAuthoredPosts: 142 }
    ],
    callnycCouncilMemberCount: 7,
    nycacCouncilMemberFloor: 5,
    callnycCouncilPostIds,
    nycacCouncilPostIds,
    excludedHandles: ["@fairrentnyc", "@sundaydinnernyc"],
    selectedPublicClaimIds: ["CLM-CALLNYC-COUNCIL-ACCOUNT-ENGAGEMENT"]
  },
  ...
  observations: [
    ...
    { id: "OBS-X-KCTH-REQUEST-ACTION-REPORTING-LOOP", intakeId: "INTAKE-KCTH-X-ARCHIVE-2026-07-14", sourceId: "SRC-X-AUDIT-PROJECT-ACCOUNTS-2026-07-14", project: "kc-town-hall", kind: "source-fact", text: "The recovered @KCTownHall timeline documents a recurring request-to-action loop: solicit tire locations and neighborhood priorities, perform pickups or public programs, then report results back through the same public identity from 2018 through 2022.", locator: "170-status recovered set, July 2, 2018-September 24, 2022", status: "corroborated", publicSafe: true, claimIds: ["CLM-KCTH-SOCIAL-SERVICE-REPORTING"], researchInquiryIds: ["INQ-PROJECT-SOCIAL-ARCHIVE-PRODUCTION-2026-07-14"], limitations: ["Project posts self-report 599 tires and $13,235 in avoided fees for 2019, more than $32,000 by November 2021, and 139 tires in May 2022; these figures need independent reconciliation before stronger public use."] }
  ],
  ...
  claims: [
    {
      ...
      project: "callnyc",
      internalClaim: "A bounded authenticated audit recovered authored public engagement with CallNYC from seven people who were sitting New York City Council members on the post dates.",
      status: "confirmed-with-boundary",
      projections: [{ key: "case-study", text: "The independent CallNYC prototype Jamie built drew public engagement from seven sitting New York City Council members through replies, quote-posts, reposts, and direct links.", status: "active", citationRequired: true, surfaces: ["/work/callnyc"] }],
      evidence: [
        { sourceId: "SRC-X-AUDIT-CALLNYC-COUNCIL-ENGAGEMENT-2026-07-14", relationship: "direct-support", supports: ["deduplicated seven-member count", "interaction types", "collection method", "exclusions and limitations"], confidence: "high", renderCitation: true },
      ...
      boundaries: ["Say seven recovered sitting-member accounts, not seven endorsements, adopters, offices using the product, or all lifetime engagement.", "Count only posts authored by the member account; project outreach tagging a member is not member engagement.", "Carlina Rivera's 2016 post is retained as public engagement but excluded because it predates her Council service."],
      antiClaims: ["Seven Council members adopted CallNYC.", "The Council endorsed CallNYC.", "Every Council-member interaction was recovered.", "Likes or repost totals identify specific members."],
    ...
    },
    {
      ...
      evidence: [{ sourceId: "SRC-X-PROFILE-NYCARTC", relationship: "direct-support", supports: ["four campaign hashtags", "shared account identity", "5,124-post accessed-at snapshot"], confidence: "high", renderCitation: false }],
      boundaries: ["Profile totals are volatile snapshots.", "Keep account output collective unless post-level authorship is established.", "Do not attribute the present @fairrentnyc account to the campaign."],
    ...
    },
    {
      ...
      internalClaim: "The recovered KC Town Hall timeline documents a recurring public request-to-action-and-reporting loop for neighborhood priorities and Tired of Tires service from 2018 through 2022.",
      status: "confirmed-with-boundary",
      projections: [{ key: "archive-note", text: "KC Town Hall used one public identity to solicit neighborhood needs, route participation, perform recurring programs, and report results back from 2018 through 2022.", status: "hold", citationRequired: false, surfaces: [] }],
      evidence: [
        { sourceId: "SRC-X-AUDIT-PROJECT-ACCOUNTS-2026-07-14", relationship: "direct-support", supports: ["170-status recovered set", "request-to-action reporting pattern", "2018-2022 continuity"], confidence: "high", renderCitation: false },
        { sourceId: "SRC-X-KCTH-TIRES-2019-YEAR-END", relationship: "corroborating", supports: ["2019 year-end reporting"], confidence: "high", renderCitation: false },
        { sourceId: "SRC-X-KCTH-TIRES-2021-YEAR-END", relationship: "corroborating", supports: ["2021 year-end continuity"], confidence: "high", renderCitation: false },
        { sourceId: "SRC-X-KCTH-TIRES-2022-MAY", relationship: "corroborating", supports: ["2022 recurring service continuity"], confidence: "high", renderCitation: false }
      ],
      boundaries: ["Keep metric figures explicitly identified as project self-reporting until independently reconciled.", "Do not expose resident addresses, phone numbers, direct messages, or private service records.", "Credit residents, neighborhood partners, volunteers, and City collaborators collectively."],
      antiClaims: ["Jamie alone delivered every pickup.", "The social timeline is an audited service ledger.", "Public posts authorize release of private resident records."],
      researchInquiryIds: ["INQ-PROJECT-SOCIAL-ARCHIVE-PRODUCTION-2026-07-14"],
      reviewedAt,
      reviewedBy: ["Jamie Burkart", "Codex authenticated-browser archival review"]
    }
