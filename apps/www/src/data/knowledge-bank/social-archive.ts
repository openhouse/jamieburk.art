...
export const socialArchiveSources: SourceRecord[] = [
  ...
  },
  {
    ...
    supportsGenerally: ["rounded 1,800+ historical user aggregate", "rounded 16,000+ historical posts/events aggregate", "35+ historically active city scenes under a 50-plus geocoded-post threshold"],
...
export const socialArchiveClaims: ClaimRecord[] = [
  ...
  },
  {
    ...
    status: "confirmed-with-boundary",
    projections: [
      { key: "case-study", text: "Archived production records support 1,800+ users, 16,000+ posts/events, and 35+ historically active city scenes, using at least 50 geocoded posts/events as the conservative scene threshold. These are historical aggregate records, not official chapters, current activity, or a complete adoption census.", status: "active", citationRequired: true, surfaces: ["/work/wowlist"] },
      { key: "homepage", text: "WOWList supported 1,800+ historical users, 16,000+ historical posts/events, and use across 35+ historically active city scenes under a conservative threshold.", status: "active", citationRequired: false, surfaces: ["/"] },
      { key: "resume-html", text: "WOWList supported 1,800+ historical users, 16,000+ historical posts/events, and use across 35+ historically active city scenes under a conservative threshold.", status: "active", citationRequired: false, surfaces: ["/resume"] },
      { key: "resume-pdf", text: "Jamie co-built WOWList with Richard Caceres; historical production records show use across 35+ active city scenes, each with at least 50 geocoded posts/events.", status: "active", citationRequired: false, surfaces: ["/resume/Jamie-Burkart-Resume-Technical-Project-Manager.pdf"] },
      { key: "technical-operations", text: "WOWList supported 1,800+ historical users, 16,000+ historical posts/events, and use across 35+ historically active city scenes under a conservative threshold.", status: "active", citationRequired: false, surfaces: ["/work/technical-operations"] },
      { key: "work-card", text: "Jamie co-built WOWList, historically used across 35+ active city scenes under a conservative threshold.", status: "active", citationRequired: false, surfaces: ["/work"] }
    ],
    evidence: [
    ...
    boundaries: ["Treat the figures as historical aggregate database records.", "Define 35+ historically active city scenes by the documented 50-plus geocoded-post threshold.", "Keep raw user, contact, geolocation, and community records protected.", "Credit the platform and adoption as collective work."],
  ...
  },
  {
    ...
    internalClaim: "The surviving KC Town Hall profile population documents a shared public request-to-action-and-reporting surface, with 100 of 183 records concerning resident tire intake, pickup operations, or result reporting.",
    status: "confirmed-with-boundary",
    projections: [{ key: "case-study", text: "KC Town Hall used its shared public account as a request-to-action-and-reporting surface: 100 of 183 surviving records concern tire reports, pickups, or results from 2019 through 2022. Those records are not an audited service total and do not assign every post or pickup to Jamie.", status: "active", citationRequired: true, surfaces: ["/work/kc-town-hall"] }],
    evidence: [{ sourceId: "SRC-SOCIAL-ARCHIVE-INVENTORY-2026-07-15", relationship: "direct-support", supports: ["183-record profile review", "100 tire-workflow records", "request-to-action reporting pattern"], confidence: "high", renderCitation: true }, { sourceId: "SRC-KCTH-X-PROFILE", relationship: "context", supports: ["dedicated account identity", "profile population"], confidence: "high", renderCitation: true }, { sourceId: "SRC-KCTH-PINNED-2018", relationship: "direct-support", supports: ["participatory restoration framing"], confidence: "high", renderCitation: false }, ...kcTownHallEngagementSourceIds.map((sourceId) => ({ sourceId, relationship: "corroborating" as const, supports: ["external public acknowledgment of neighborhood stewardship or redevelopment communication"], confidence: "high" as const, renderCitation: false }))],
    boundaries: ["Treat the account as a shared project identity rather than proof that Jamie authored every record.", "Keep project-reported totals separate from independent audit.", "Do not expose resident addresses, phone numbers, direct messages, or private service records."],
    antiClaims: ["Jamie alone authored the account or delivered every pickup", "The social timeline is an audited service ledger", "One hundred records equal one hundred completed pickups or households", "Social activity proves project completion or funding receipt"],
    researchInquiryIds: ["INQ-SOCIAL-KCTH-ARCHIVE", "INQ-SOCIAL-ACCOUNT-AUTHORSHIP"],
    reviewedAt: socialArchiveReviewedAt,
    reviewedBy: ["Jamie Burkart", "Codex authenticated public-web review"]
  },
  {
...
export const socialArchiveResearchInquiries: ResearchInquiry[] = [
  ...
  },
  {
    ...
    publicSummary: "A protected production-database analysis supports a bounded historical scale claim: 1,800+ historical users, 16,000+ historical posts/events, and 35+ historically active city scenes under a documented conservative threshold."
  },
  {
  ...
  },
  {
    ...
    project: "kc-town-hall",
    question: "What public project and neighborhood-stewardship work is recoverable from @KCTownHall without exposing contact data or overstating completion?",
    methods: ["Reviewed the full surviving profile population and dated public project posts.", "Separated account-authored posts, replies, and reposted external posts.", "Excluded phone numbers, addresses, private messages, and session data."],
    runAt: socialArchiveReviewedAt,
    resultStatus: "partially-recovered",
    findings: ["The surviving 183-record profile population was recovered.", "One hundred records concern resident tire intake, pickup operations, or result reporting.", "The account publicly framed participatory restoration and repeatedly operated a resident request-to-action-and-reporting workflow."],
    limitations: ["The social record does not establish individual post authorship, independently audited service outcomes, funding receipt, or project completion.", "Historical contact details are deliberately omitted."],
    sourceIds: ["SRC-SOCIAL-ARCHIVE-INVENTORY-2026-07-15", "SRC-KCTH-X-PROFILE", "SRC-KCTH-PINNED-2018", ...kcTownHallEngagementSourceIds],
    publicSummary: "The surviving KC Town Hall account population documents a public stewardship and service-routing pattern while leaving authorship and audited outcomes open."
  },
  {
