...
My work is digital, but it does not stop at the screen. Tired of Tires connected outreach and intake to scheduled collection and a City recycling handoff. WOW List replaced a complex event form with one collaboratively editable text field, then joined that interface to onboarding, support, instrumentation, and production operations. The hinge is the same in each case: a feature is only as useful as the next action it makes possible.
