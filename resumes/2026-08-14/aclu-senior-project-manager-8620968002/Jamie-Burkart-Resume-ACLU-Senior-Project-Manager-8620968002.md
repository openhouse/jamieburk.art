# Jamie Burkart

**Senior Project Manager | Systems Implementation, Business Analysis & Organizational Learning**

Brooklyn, NY | (816) 728-8685 | [jamie.burkart@gmail.com](mailto:jamie.burkart@gmail.com)

[Portfolio](https://jamieburk.art/) | [LinkedIn](https://www.linkedin.com/in/jamie-burkart/) | [GitHub](https://github.com/openhouse)

## Professional Summary

Senior project and technical operations manager with 14+ years translating strategy, values, compliance needs, and ambiguous stakeholder concerns into executable implementation plans, public-facing systems, improved workflows, and durable organizational knowledge. Leads complex cross-department and multi-organization work through scope, requirements, sequencing, risk, communications, rollout, training, closeout, and postmortem learning. Combines business analysis, technical implementation, facilitation, clear writing, confidentiality, accessibility-aware delivery, and a strong commitment to civil liberties, public participation, and inclusive operations.

## Core Skills

- **Project strategy and delivery:** Project charters, scope and deliverables, work plans, schedules, milestones, risk and issue management, stakeholder communications, rollout, training, closeout, and retrospectives.
- **Business analysis and process improvement:** Stakeholder interviews, needs synthesis, requirements, current-state and future-state process mapping, workflow design, decision records, acceptance criteria, and implementation documentation.
- **Organizational standards and learning:** Reusable templates, shared project practices, facilitation, documentation architecture, resource libraries, onboarding materials, source maps, and training and handoff systems.
- **Responsible implementation:** Confidentiality, privacy-conscious information handling, accessibility-aware communication, public-safety review, collective-credit boundaries, compliance coordination, and candid documentation of risk and unknowns.

## Professional Experience

### THICK ARTS — Founder, Senior Project Manager & Web Systems Lead

New York, NY / Remote | 2012–Present

- Lead complex web, information, and operating-system implementations from discovery through business requirements, information architecture, technical scope, delivery plans, testing, rollout, adoption, maintenance, and handoff.
- Facilitate stakeholder alignment and translate different professional languages into usable plans, interfaces, workflows, training materials, decision logs, and accountable next steps.
- Led long-running e-commerce, analytics, content, marketing, and operational improvements for Harry J. Epstein Company, contributing to a period of 2x revenue growth while helping an 80+ year-old legacy business modernize its customer and internal workflows.

### NYC Artist Coalition / FairRentNYC — Co-Founder, Civic Systems, Coalition Operations & Policy Communications Lead

New York, NY | 2017–Present

- Help translate stakeholder listening, public testimony, policy research, and coalition priorities into executable campaign plans, public forums, websites, communications, calls to action, and sustained government-facing follow-through.
- Built and stewarded 30+ pages of shared campaign memory for a 2026 Commercial Rent Stabilization collaboration, connecting running minutes, decisions, action items, legal and policy questions, media assets, public-data framing, and city/state strategy lanes.
- Facilitate challenging meetings; brief and work directly with elected officials and staff; maintain tact and confidentiality across public and protected information; and represent collective work in testimony, presentations, and press without claiming sole campaign outcomes.

### KC Town Hall LLC — Co-Founder & Project Manager, Historic Restoration / Mixed-Use Development

Kansas City, MO | 2015–2024

- Co-led strategy, planning, public-benefit documentation, vendor coordination, field implementation, and municipal compliance for adaptive reuse of an approximately 6,500 sq. ft. historic building into four commercial spaces and three homes.
- Co-developed the successful CCED proposal and served as the City's named developer/presenter for a $490,539 public funding request; remained a documented point of contact through M/WBE and CREO compliance progress and later transitioned the project to a mission-aligned organization.

### WOWList.org — Co-Founder, Product & Community Systems

New York, NY / Remote | 2015–2020

- Co-founded, co-built, and operated with Richard Caceres a community-calendar platform, coordinating technical work, organizer needs, public communication, onboarding, support, adoption, and distributed maintenance across approximately 35 city ecosystems.

### CallNYC.org — Civic-Tech Builder & Product Translator

New York, NY | 2016

- Built and launched an independent resident-facing civic prototype that translated CouncilStat constituent-services open data into issue pathways, district context, and next-step guidance; covered by Politico New York in 2016.

### 196 Artists Residency / Sunday Dinner — Founder & Systems Steward

Brooklyn, NY | 2012–Present

- Created repeatable invitation, onboarding, facilitation, documentation, and continuity systems across 300+ gatherings and 20+ resident artists, balancing durable learning with participant privacy and care.

## Education & Professional Development

**University of California, Santa Cruz** — B.A., Film & Digital Media, Honors

**AI Evals for Engineers & PMs** — Shreya Shankar & Hamel Husain / Maven, 2026; evidence-based evaluation, error analysis, annotation workflows, retrieval quality, and human review.

## Additional Information

Authorized to work in the United States; no sponsorship required. Valid New York State driver's license.
