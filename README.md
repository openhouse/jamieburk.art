...
sources, research questions, decisions, and selective portfolio projections:
...
`npm run knowledge-wiki` checks the governed Markdown graph; `npm run
evals:wiki` runs the current Wiki evaluation suite.
`npm run knowledge-bank` remains a compatibility command for the proof schema,
projection rules, and work
...
begin as versioned proposals under [`rfcs/`](./rfcs/README.md). Merging a
proposal in `proposed` or `exploring` state preserves the idea for discussion; it
does not authorize implementation. Run `npm run check:rfcs` to validate the RFC
...
`npm run check:rfps` remains a deprecated compatibility alias for older local
automation.
