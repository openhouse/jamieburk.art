...
metadata. `npm run check:citations` checks source, claim, citation-group,
research, media, correction, and page-order integrity. `npm run
test:citations` runs deterministic citation regressions. `npm run
report:citations` writes a review report under `reports/`. `npm run
check:routes` checks canonical routes and legacy redirects.
