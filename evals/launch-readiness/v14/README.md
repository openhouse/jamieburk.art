# Launch-readiness evals v14

This suite turns the portfolio's publication intentions into an optimization
contract for human and LLM collaborators.

Version 13 adds a deterministic NYC Artist Coalition X archival-production
gate: complete disposition of the dated profile-count population, recovered
post and URL controls, bounded stakeholder-exchange floors, and an explicit
owner-archive dependency for unresolved items. It retains the v12 KC Town Hall
population gate, the v11 WOW List population gate, the v10 Google Shared Drive
gate, the v9 iCloud gate, the v8 KC Town Hall stewardship gate, and every
earlier invariant. Versions 1 through 12 remain in the repository with their
original weights and observation formats so earlier candidate records stay
reproducible.
...
```bash
cp evals/launch-readiness/v14/observation-template.json \
...
Read evals/launch-readiness/v14/evals.json and preserve every protected
...
Version 13 adds a deterministic NYC Artist Coalition X archival-production
gate. It recomputes the complete 5,124-slot disposition and the bounded inbound
stakeholder ledger, verifies eight close-read source records and three held
claims, and preserves the first-party owner-archive dependency. Full slot
disposition must never be renamed full item-level tweet recovery.
