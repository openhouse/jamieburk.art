# Launch-readiness evals v5

This suite turns the portfolio's publication intentions into an optimization
contract for human and LLM collaborators.

Version 5 adds a deterministic campaign-press gate. It preserves all 45 article
placements recovered from four NYC Artist Coalition campaign sites as 44
canonical sources while enforcing the boundary between campaign-site placement,
article-level close reading, and editorial selection. Versions 1 through 4
remain in the repository with their original weights and observation formats so
earlier candidate records stay reproducible.
...
```bash
cp evals/launch-readiness/v5/observation-template.json \
...
Read evals/launch-readiness/v5/evals.json and preserve every protected
