# Blind-spot hill-climb log

## Attempt 1: selection and present relevance

- **Observed risk:** The homepage Start here section offered five parallel
  destinations, and the Technical Operations proof map presented eight
  projects before its capability evidence.
- **Hypothesis:** A smaller route choice and four complementary proofs will
  reduce reader burden without deleting archive depth.
- **Bounded change:** Reduce the homepage quick path to Technical Operations,
  selected work, and resume; reduce the Technical Operations opening proof set
  to HJE, FairRentNYC, CallNYC, and Source-Backed Team Memory; add one explicit
  present-tense role-fit sentence.
- **Protected boundaries:** Keep every project in the Work index and knowledge
  bank; do not strengthen any factual claim.
- **Result:** Typecheck, lint, production build, public safety, routes, and the
  full repository check pass. Semantic and human observations remain pending.
  Exact browser observation was attempted, but the managed workspace refused
  the localhost listening socket; browser criteria remain unobserved rather
  than inferred from the build.

## Stop boundary

The repository can prepare reader, collaborator, holdout, rights, and approval
work. It cannot mark those observations complete without the required people.

## Attempt 2: explicit observer ownership

- **Observed failure:** `BLINDSPOT-001` rejected two human or rights records
  whose stop boundaries described prohibited action without naming who owns the
  decision.
- **Hypothesis:** Explicitly stating that an agent cannot clear rights or serve
  as the human holdout will remove ambiguity without weakening the boundary.
- **Bounded change:** Strengthen the visual-proof and holdout stop boundaries.
...
# v24 composite integration

## Baseline: `5a7095ff`

- **Starting state:** `feature/knowledge-d` matched the frozen
  `feature/evals-D` head. The v23 deterministic layer passed 22 of 26 hard
  gates; browser, semantic, runtime, and human evidence was not supplied in the
  baseline report. The complete repository check passed under Node 26.5.0.
- **Constraint:** Preserve every v23 criterion and historical file, the
  canonical knowledge records, public-safety boundaries, and the composed
  website. Do not alter any `feature/evals-*` branch.

## Attempt 1: bind evidence to candidate, contract, and observer

- **Observed failure:** The v23 runner accepted an observation for any full Git
  SHA, did not bind it to a contract fingerprint or current checkout, counted
  distinct run IDs as independent semantic graders, and could accept a
  threshold-inconsistent `passed: true` flag.
- **Hypothesis:** A canonical fingerprint plus exact-candidate and observer
  validation will make recursive scores auditable without changing portfolio
  prose or weakening human stop boundaries.
- **Bounded change:** Add one active-version pointer; create v24; require exact
  current SHA, contract fingerprint, stable grader identity, correct observer
  type, optimizer independence, and non-agent human status; add adversarial
  tests; retain a runnable v23 command.
- **Result:** `EVALSYS-001` passes. Six adversarial evaluator tests pass. The
  active contract reports 45 criteria while v23 remains unchanged and runnable.

## Attempt 2: make canonical knowledge operable

- **Observed failure:** The knowledge bank was structurally rich but a newcomer
  had to read many TypeScript modules to find a record, understand lifecycle
  totals, or inspect active projection state.
- **Hypothesis:** Derived operator views can reduce maintenance burden if they
  read canonical records directly, omit protected evidence detail, and fail on
  drift.
- **Bounded change:** Add ID/project/text query, lifecycle report, cross-record
  validation, and a generated projection register. Do not add a database,
  schema, public route, or automatic publication behavior.
