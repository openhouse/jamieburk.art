# Launch-readiness evals v2

This suite turns the portfolio's publication intentions into an optimization
contract for human and LLM collaborators.

Version 2 adds `CHAD-001`. Version 1 remains in the repository with its original
weights and observation format so the earlier candidate record stays
reproducible.
...
```bash
cp evals/launch-readiness/v2/observation-template.json \
...
Read evals/launch-readiness/v2/evals.json and preserve every protected
