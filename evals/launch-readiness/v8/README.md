# Launch-readiness evals v8

This suite turns the portfolio's publication intentions into an optimization
contract for human and LLM collaborators.

Version 8 strengthens the deterministic KC Town Hall gate by separating the
official municipal sequence from Jamie's first-person confirmation that he
later transitioned the project to a mission-aligned organization. It requires
the public-safe transition while withholding the organization's identity,
transfer mechanics, present status, and private circumstances. It retains the
v7 interim-status gate, the v6 Council-action gate, the v5 campaign press gate,
and every earlier invariant. Versions 1 through 7 remain in the
...
```bash
cp evals/launch-readiness/v8/observation-template.json \
...
Read evals/launch-readiness/v8/evals.json and preserve every protected
