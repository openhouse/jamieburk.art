# Launch-readiness evals v4

This suite turns the portfolio's publication intentions into an optimization
contract for human and LLM collaborators.

Version 4 adds a deterministic research-batch gate for ten newly recovered
public sources and defines decomposition over every reviewed source rather than
the original five-source pilot. Versions 1 through 3 remain in the repository
with their original weights and observation formats so earlier candidate
...
```bash
cp evals/launch-readiness/v4/observation-template.json \
...
Read evals/launch-readiness/v4/evals.json and preserve every protected
