# Launch-readiness evals v10

This suite turns the portfolio's publication intentions into an optimization
contract for human and LLM collaborators.

Version 10 adds a deterministic archival-production gate for Google Shared
Drives. It requires a governed inventory, selective close reading, protected
Drive locators, revision-aware attribution, explicit support limits, and a
small editorial selection. It keeps research drafts, authored proposals,
visual-only holdings, sensitive records, and unverified aggregate counts from
becoming public proof. It retains the v9 iCloud archival-production gate, the
v8 KC Town Hall stewardship gate, the v7 interim status gate, the v6
Council-action gate, the v5 campaign press gate, and every earlier invariant.
Versions 1 through 9 remain in the
repository with their original weights and observation formats so earlier
candidate records stay reproducible.
...
```bash
cp evals/launch-readiness/v10/observation-template.json \
...
Read evals/launch-readiness/v10/evals.json and preserve every protected
