...
through explicit stages, and kept as part of the project's history. RFCs may
concern research practice, editorial governance, privacy, and organizational
design as well as software interfaces.
...
The former `rfps/` name was an early typo. `npm run check:rfps` is retained only
as a deprecated command alias while local automation migrates.
